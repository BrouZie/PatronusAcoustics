...
/* The packet carries trailing padding so its length suits the USART MDMA; the
 * viewer skips it, since the header states the map size and the reader
 * resynchronises on the magic word. */
...
/* Frames between transmitted maps, derived rather than guessed: 8N1 puts 10
 * bits on the wire per byte, and aiming at 60% of the link leaves each send
 * comfortably finished before the next is due. A one-row grid comes out every
 * frame, the full sphere grid every tenth, with no constant to keep in step. */
...
#define SRP_STREAM_EVERY_N_FRAMES ((SRP_PACKET_BYTES * SRP_FRAMES_PER_SEC + SRP_STREAM_BUDGET - 1U) / SRP_STREAM_BUDGET)

/* The MDMA source must live in DTCM: the D-cache is on and no MPU region covers
 * .d1_buf, so a packet staged there would sit in cache while the MDMA read
 * stale SRAM. */
static struct
{
...
_Static_assert(CONSOLE_PAYLOAD_OK(sizeof(_packet)),
               "SRP packet is not a sendable payload -- coarsen the grid, see console.h");

_Static_assert(AUDIO_MIC_COUNT >= 2,
               "srp-phat needs at least two microphones -- set AUDIO_MIC_COUNT in audio_config.h");
